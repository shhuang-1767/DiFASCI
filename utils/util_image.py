...
def shuffle_crop(train_data, crop_size=256):
    img = train_data
    h, w, _ = img.shape
...
"--------------------------KAIST----------------------------"
def LoadKAISTTraining(path):
    img = sio.loadmat(path)
    if "HSI" in img:
       img = img["HSI"].astype(np.float32)
       img[img < 0] = 0
       img[img > 1] = 1
    elif "HSI" in img:
       img = img["HSI"].astype(np.float32)
...
"--------------------------CAVE----------------------------"
def LoadCAVETraining(path):
    img = sio.loadmat(path)
    if "img_expand" in img:
       img = img["img_expand"].astype(np.float32) / 65536.
    elif "img" in img:
       img = img["img"].astype(np.float32) / 65536.
    #print('Scene {} is loaded. {}'.format(i, scene_list[i]))
    return img

def LoadCAVETesting(path):
    img = sio.loadmat(path)
    if "img_expand" in img:
       img = img["img_expand"].astype(np.float32)
    elif "img" in img:
       img = img["img"].astype(np.float32)
    #print('Scene {} is loaded. {}'.format(i, scene_list[i]))
    return img

def load_file_list(file_list_path: str):
    files = []
    with open(file_list_path['file_list'], "r") as fin:
        for line in fin:
            path = line.strip()
            if path:
                files.append(path)
...
"--------------------------NTIRE----------------------------"
def LoadNTIRETraining(path):
    with h5py.File(path, 'r') as f:
        # Assume the mat file contains a group 'cube' with the image data
        if 'cube' in f:
            img = np.array(f['cube']).astype(np.float32)
            img = img.transpose(1,2,0)[:,:,0:28]
        return img

def crop(orig_matrix):
      center_height = orig_matrix.shape[0] // 2
      center_width = orig_matrix.shape[1] // 2

      # 计算截取区域的起始和结束索引
      start_height = center_height - 128  # 258 的一半是 129，但需要向前多走一步来包含中心
      end_height = center_height + 128
      start_width = center_width - 128
      end_width = center_width + 128

      # 截取中心的 (258, 258, 28) 部分
      
      # 裁剪矩阵
      cropped_matrix = orig_matrix[start_height:end_height, start_width:end_width, :]
      return cropped_matrix


def LoadNTIRETesting(path):
    with h5py.File(path, 'r') as f:
        # Assume the mat file contains a group 'cube' with the image data
        if 'cube' in f:
            img = np.array(f['cube']).astype(np.float32)
            img = crop(img.transpose(1,2,0))[:,:,:28]
            img = np.transpose(img, (2, 0, 1))
        return torch.from_numpy(img)
    
"--------------------------ICVL----------------------------"
def LoadICVLTraining(path):
  with h5py.File(path, 'r') as f:
    # Assume the mat file contains a group 'cube' with the image data
    if 'rad' in f:
        img = np.array(f['rad']).astype(np.float32)
        img = img.transpose(1,2,0)[:,:,0:28] / img.max()
        #img = np.rot90(img, 2, axes=(0, 1))
    return img
  
def LoadICVLTesting(path_test):
    img = sio.loadmat(path_test)['gt'][:,:,0:28]
    img = crop(img)
...
#   # Assume the mat file contains a group 'cube' with the image data
#   if 'rad' in f:
#       img = np.array(f['rad'])[0:28, :, :].transpose(1,2,0).astype(np.float32)
#       img = crop(img)
#       img = img.transpose(2,0,1) / img.max()
#       #img = np.rot90(img, 2, axes=(0, 1))
#   return torch.from_numpy(img)  

"--------------------------Harvard----------------------------"
def LoadHarvardTraining(path):
    img_dict = sio.loadmat(path)
    if "gt" in img_dict:
        img = img_dict['gt'][:,:,0:28]
    if "ref" in img_dict:
        img = img_dict['ref'][:,:,0:28]
        img = crop(img).astype(np.float32)
    return img
  
def LoadHarvardTesting(path):
    img = sio.loadmat(path)['gt'][:,:,0:28]
    img = crop(img).astype(np.float32)
    img = np.transpose(img, (2, 0, 1))
    return torch.from_numpy(img)
