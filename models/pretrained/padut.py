...
class PADUT(nn.Module):
    def __init__(self, in_c=28,  n_feat=28,  nums_stages=2,n_depth=3, pretrained_model_path = False):
        ...
        if pretrained_model_path != False:
            ...
            #checkpoint = torch.load(pretrained_model_path)["model"]
            checkpoint = torch.load(pretrained_model_path)
            if "model" in checkpoint:
                checkpoint = checkpoint["model"]
            if "pretrained" in checkpoint:
              checkpoint = torch.load(pretrained_model_path)['pretrained']
