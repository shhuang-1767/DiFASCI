...
class SSRNet(torch.nn.Module):
    def __init__(self, stage = 9, bands = 28, size = 256, pretrained_model_path = False):
        ...
        if pretrained_model_path != False:
    ...
    def mul_PhiTg(self, Phi_shift, g):
        temp_1 = g.repeat(1, Phi_shift.shape[1], 1, 1).to(Phi_shift.device)
    ...
    def forward(self, g, input_mask=None):
        ...
        temp_g = g_normal.unsqueeze(1).repeat(1, 28, 1, 1)
        ...
        for i in range(self.stage):
            ...
            z = f + rho * self.mul_PhiTg(Phi_shift, torch.div(g.unsqueeze(1) - Phi_f, PhiPhiT.unsqueeze(1)))
            '''SSRU'''
